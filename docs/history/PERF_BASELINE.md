...
✅ **GPU process spike confirmed normal (no leak).** Owner ran
   the full B.2 cycle test 2026-05-22 PM:
     - Fresh launch baseline: 35.2 MB
     - During active scrubber: 110.5 MB (+75 MB decode pool)
     - After multiple scrubber → nav-away cycles: **59.9 MB**
   The 59.9 MB reading is LOWER than the single-cycle 67 MB
   reading — clear evidence the GPU process actively releases
   memory it isn't using. If this were a per-scrubber-session
   leak, the number would have climbed (~90 / ~120 / ~150 MB)
   instead of decreasing.
   The persistent ~25 MB above pristine baseline is a one-time
   GPU decode pool allocation by WebView2 + Windows DXVA. The
   OS keeps it reserved for re-use across video sessions. Not
   our memory to free. Closed.
